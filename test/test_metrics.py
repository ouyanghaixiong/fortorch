...
class Test(TestCase):
    def test_dcg_at_n(self):
        scores = torch.tensor([[4], [1], [0], [2]], dtype=DTYPE)
        dcg = dcg_at_n(scores, k=4)
        self.assertTrue(dcg > 0)
        self.assertIsInstance(dcg, float)

    def test_ndcg_score_i(self):
        y_pred = torch.tensor([[4], [1], [0], [2]], dtype=DTYPE)
        y_true = torch.tensor([[0], [0], [4], [3]], dtype=DTYPE)
        ...
        y_pred = torch.tensor([[4], [1], [0], [2]], dtype=DTYPE)
        y_true = torch.tensor([[4], [1], [0], [2]], dtype=DTYPE)
        ndcg = ndcg_score_i(y_pred, y_true)
        self.assertIsInstance(ndcg, float)
        self.assertTrue(0 <= ndcg <= 1)
        self.assertEqual(round(ndcg, 4), 1.0)

        y_pred = torch.tensor([[0], [1], [2], [3]], dtype=DTYPE)
        y_true = torch.tensor([[4], [3], [2], [1]], dtype=DTYPE)
        ndcg = ndcg_score_i(y_pred, y_true)
        self.assertIsInstance(ndcg, float)
        self.assertTrue(0 <= ndcg <= 1)
    ...
    def test_ndcg_score(self):
        y_pred0 = torch.tensor([[4], [1], [0], [2]], dtype=DTYPE)
        y_true0 = torch.tensor([[0], [0], [4], [3]], dtype=DTYPE)

        y_pred1 = torch.tensor([[4], [1], [0], [2]], dtype=DTYPE)
        y_true1 = torch.tensor([[4], [1], [0], [2]], dtype=DTYPE)

        y_pred2 = torch.tensor([[0], [1], [2], [3]], dtype=DTYPE)
        y_true2 = torch.tensor([[4], [3], [2], [1]], dtype=DTYPE)
